...
import type { DirectusDemo, Demo, FetchDemosResponse, Locale } from '../src/types'
const client = createDirectus('https://fari-cms.directus.app').with(rest())

export async function fetchDemos(): Promise<FetchDemosResponse> {
  try {
      ...
      .map((demo: DirectusDemo, index): Demo | null => {
        ...
        for (const translation of translations) {
          ...
          if (
            ...
            app_url.match(/^(\/|file:)/) || // File paths
            app_url.match(/\d+\.\d+\.\d+\.\d+/) // IP addresses
          ) {
